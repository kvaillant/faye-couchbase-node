...
Engine.prototype = {
  ...
  destroyClient: function (clientId, callback, context) {
    ...
    self._couchbase.get(self._ns + '/clients/' + clientId + '/channels', function (error, result) {
      if (error) {
        self._server.error('Client destroy :', error, clientId);
      }
      if (result && result.value) {
        ...
        async.each(channels, function (channel, callbackEach) {
          self._couchbase.get(self._ns + '/channels' + channel, function (error, result) {
            if (error) {
              self._server.error('Client destroyClient [GET]:', error, clientId);
            }
            var dataClients = {
              clients: []
            };
          ...
          self._couchbase.remove(self._ns + '/clients/' + clientId + '/channels', function (error, result) {
            if (error) {
              self._server.error('Client destroyClient /clients/?/channels [REMOVE]:', error, clientId);
            }
          });
          self._couchbase.remove(self._ns + '/clients/' + clientId + '/messages', function (error, result) {
            if (error) {
              self._server.error('Client destroyClient /clients/?/messages [REMOVE]:', error, clientId);
            }
          });
          self._couchbase.remove(this._ns + '/clients/' + clientId, function (error, result) {
            if (error) {
              self._server.error('Client destroyClient /clients/? [REMOVE]:', error, clientId);
            }
          });
  ...
  subscribe: function (clientId, channel, callback, context) {
    ...
    async.waterfall([
      function (callbackW) {
        self._couchbase.get(self._ns + '/clients/' + clientId + '/channels', function (error, result) {
          if (error) {
            self._server.error('Client subscribe [GET]:', error, clientId, channel, channel, dataChannels.channel);
          }
          var dataChannels = {
            channels: []
          };
      ...
      function (arg, callbackW) {
        self._couchbase.get(self._ns + '/channels' + channel, function (error, result) {
          if (error) {
            self._server.error('Client subscribe [GET]:', error, channel);
          }
          var dataClients = {
            clients: []
          };
  ...
  unsubscribe: function (clientId, channel, callback, context) {
    ...
    async.waterfall([
      function (callbackW) {
        self._couchbase.get(self._ns + '/clients/' + clientId + '/channels', function (error, result) {
          if (error) {
            self._server.error('Client unsubscribe [GET]:', error, clientId, channel);
          }
          var dataChannels = {
            channels: []
          };
      ...
      function (arg, callbackW) {
        self._couchbase.get(self._ns + '/channels' + channel, function (error, result) {
          if (error) {
            self._server.error('Client unsubscribe [GET]:', error, channel);
          }
          var dataClients = {
            clients: []
          };
          ...
          if (indexC !== -1) {
            ...
            if (dataClients.clients.length === 0) {
              self._couchbase.remove(self._ns + '/channels' + channel, function (error, result) {
                if (error) {
                  self._server.error('Client unsubscribe [DELETE]:', error, channel);
                }
                callbackW(null, 'done');
              });
            } else {
  ...
  emptyQueue: function (clientId, callback) {
    ...
    self._couchbase.get(key, function (error, result) {
      if (error) {
        self._server.error('Client emptyQueue [GET]:', error);
      }

...
var Engine = function (server, options) {
  ...
  this._gc = gc < (60 * 60 * 24 * 7) ? gc : this.DEFAULT_GC;
...
Engine.prototype = {
  ...
  DEFAULT_GC: 60 * 60, // 1hour session
  LOCK_TIMEOUT: 120,
  ...
  emptyQueue: function (clientId, callback) {
    ...
    self._couchbase.get(key, function (error, result) {
      ...
      if (result && result.value) {
        self._couchbase.remove(key, function (error, result) {
          if (error) {
            self._server.warn('Client emptyQueue [DELETE]:  ?,  ?', error, key);
          }

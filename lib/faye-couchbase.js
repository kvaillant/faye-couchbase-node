...
Engine.prototype = {
  ...
  createClient: function (callback, context) {
    ...
    self._server.info('Client creation, sessionExpiry:  ?', sessionExpiry);
    this._couchbase.insert(this._ns + '/clients/' + clientId, true, { 'expiry': sessionExpiry }, function (error, res) {
      ...
      self._server.info('Created new client ?', clientId);
      self.ping(clientId);
    ...
    self._couchbase.get(self._ns + '/clients/' + clientId + '/channels', function (error, result) {
      ...
      if (result && result.value) {
        ...
        async.each(channels, function (channel, callbackEach) {
          ...
          self._server.info('Destroyed client ?', clientId);
  ...
  ping: function (clientId) {
    ...
    this._server.info('Ping ?, ?', clientId, timeout);
  ...
  subscribe: function (clientId, channel, callback, context) {
    ...
    async.waterfall([
      ...
      self._server.info('Subscribed client ? to channel ?', clientId, channel);
      if (callback) callback.call(context);
    });
  },
  ...
  unsubscribe: function (clientId, channel, callback, context) {
    ...
    async.waterfall([
      ...
      self._server.info('Unsubscribed client ? from channel ?', clientId, channel);
      if (callback) callback.call(context);
    });
  ...
  publish: function (message, channels) {
    this._server.info('Publishing message ?', message);

    var self = this,
      keys = channels.map(function (c) { return self._ns + '/channels' + c; });
    this._server.info('Channel for message publication ?', channels);

    self._couchbase.getMulti(keys, function (error, result) {
      self._server.info('Channels\' clients for message publication ?', result);
      if (result) {
        var clients = [];
        for (var i in result) {
        ...
        async.each(clients, function (clientId, callback) {
          ...
          self._server.info('Queueing for client ?: ?', clientId, message);

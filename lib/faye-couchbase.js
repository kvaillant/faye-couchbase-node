...
Engine.prototype = {
        ...
        channels.forEach(function (channel) { // Suppress client from channel clients list
          self._couchbase.get(self._ns + '/channels' + channel, function (error, result) {
            ...
            if (indexC !== -1) {
              ...
              self._couchbase.upsert(self._ns + '/channels' + channel, dataClients, { 'expiry': self._gc });
            }
            if (dataClients.clients.length === 0) {
              self._couchbase.remove(self._ns + '/channels' + channel);
            }
          });
        });
      }
      self._couchbase.remove(self._ns + '/clients/' + clientId + '/channels');
      self._couchbase.remove(self._ns + '/clients/' + clientId + '/messages');
      self._couchbase.remove(this._ns + '/clients/' + clientId);
  ...
  ping: function (clientId) {
    ...
    this._couchbase.upsert(this._ns + '/clients/' + clientId, true, { 'expiry': sessionExpiry });
  },
  ...
  subscribe: function (clientId, channel, callback, context) {
    ...
    self._couchbase.get(self._ns + '/clients/' + clientId + '/channels', function (error, result) {
      ...
      if (dataChannels.channels.indexOf(channel) === -1) {
        dataChannels.channels.put(channel);
      ...
      self._couchbase.get(self._ns + '/channels' + channel, function (error, result) {
        ...
        if (dataClients.clients.indexOf(clientId) === -1) {
          dataClients.clients.put(clientId);
